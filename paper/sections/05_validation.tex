...
statistic or GEMM operand sees them. Tail weight is excess kurtosis (Fisher's
convention, Gaussian $= 0$), computed over the full flattened tensor.

Each layer's $k$ maps to an equivalent tail index by $\nu = 4 + 6/k$, the inverse
of a t-Student's $k = 6/(\nu - 4)$, defined only for $\nu > 4$; the out-of-domain
case ($k \le 0$) did not arise at any layer, though h6's $\nu = 4.06$ sits within
0.06 of the boundary. The quantized-GEMM pipeline of
...
distinct tokens. Within the top-1\% pool the same channel holds 13.0\% of
the mass and appears for all 34,390 tokens, and the top ten channels hold
75.3\%; per-channel maxima drop sharply after a handful (8.06, 7.08, 6.55, 5.31,
then 2.76 for the next channel down). This is
...
finds it sound at two of the three layers tested in this one small model ---
failing only under extreme, channel-concentrated kurtosis, where a single
kurtosis-matched $\nu$ is insufficient.
