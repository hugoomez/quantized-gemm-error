...
> **Anonymized supplementary copy.** This is the preregistration document
> referenced from the paper's Method section and NeurIPS checklist. It is
> reproduced verbatim from the version-controlled original (git tag
> `preregistration-v1`), with no author names, affiliations, or repository
> URLs present in the source and none added here. Internal references to
> `README.md`, `SPEC.md`, and `configs/` point to files in the same code
> repository, which will be released (de-anonymized) with the camera-ready
> version.
